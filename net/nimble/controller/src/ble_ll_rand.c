...
uint8_t g_ble_ll_rnum_buf[MYNEWT_BLE_LL_RNG_BUFSIZE];

#define IS_RNUM_BUF_END(x)  \
    (x == &g_ble_ll_rnum_buf[MYNEWT_BLE_LL_RNG_BUFSIZE - 1])
...
ble_ll_rand_sample(uint8_t rnum)
{
    ...
    if (g_ble_ll_rnum_data.rnd_size < MYNEWT_BLE_LL_RNG_BUFSIZE) {
    ...
    while (len != 0) {
        ...
        if (len) {
            while ((g_ble_ll_rnum_data.rnd_size < len) &&
                   (g_ble_ll_rnum_data.rnd_size < MYNEWT_BLE_LL_RNG_BUFSIZE)) {
                /* Spin here */
            }
        }
    }
